...
let selectedAccount;
let kojitoken = "0x7eb567f5c781ee8e47c7100dc5046955503fc26a"; //bsc test KOJI
...
let liquidpair = "0xab2fed8c6fc7b9d0689c80bcd872c867b0551213";
let oracle = "0x2E7bC3122009E9d6487a9b62465C5Ecc5466E81e";
		   ...
		   if (apipull == 1) {
	        ...
	        paircontract.methods.getReserves().call(function(err,res){
			    if(!err){
			    	...
			    	var pooledbnb = parseFloat(web3.utils.fromWei(res._reserve1)).toFixed(2);

			    	document.getElementById("pool-bnb").innerHTML = ""+pooledbnb.replace(/\B(?=(\d{3})+(?!\d))/g, ",")+" <span>BNB<span>";

			    	var pooledkoji = parseFloat(+res._reserve0/1e9).toFixed(2);

			    	document.getElementById("pool-koji").innerHTML = "<strong>"+pooledkoji.replace(/\B(?=(\d{3})+(?!\d))/g, ",")+"<span> KOJI</span>";
			        
			       var poolbnbusdvalue = parseFloat(web3.utils.fromWei(res[1])*bnbusd).toFixed(2);

			       var kojiusdvalue = parseFloat(poolbnbusdvalue / web3.utils.fromWei(res[0],"gwei")).toFixed(9);
